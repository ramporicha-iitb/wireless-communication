...
data = pyitpp.itload('scalar_param_quantization_2bit.it')
ber = data.get('ber')
ebn0_dB = data.get('ebno_dB')
plt.plot(ebn0_dB, ber, '-*')

data = pyitpp.itload('scalar_param_quantization_3bit.it')
ber = data.get('ber')
ebn0_dB = data.get('ebno_dB')
plt.plot(ebn0_dB, ber, '-x')

data = pyitpp.itload('scalar_param_quantization_4bit.it')
ber = data.get('ber')
ebn0_dB = data.get('ebno_dB')
plt.plot(ebn0_dB, ber, '-o')
...
plt.title('2x2 MIMO-OFDM Precoding (scalar parameter quantization)')
plt.xlabel('Eb/N0 (dB)')
plt.ylabel('BER')
plt.yscale('log')
plt.legend(['2bit quantizer', '3bit quantizer', '4bit quantizer'], loc='lower left')
...
plt.ylabel('BER')
plt.yscale('log')
plt.legend(['unquantized', 'vector quantizer - 4bit', 'scalar quantizer - 4bit'], loc='lower left')
plt.grid(True)
plt.show()
